...
const Home = () => {
  ...
  const heroImg = s.heroImageUrl || campusHero;

  return (
    <>
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
        <img src={heroImg} alt={s.siteTitle} className="absolute inset-0 w-full h-full object-cover object-center" />
        ...
        <div className="relative z-10 container mx-auto px-6 text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <Link to={s.heroButton1Link} className="bg-yellow-400 hover:bg-yellow-300 text-black px-8 py-3 rounded text-sm font-body font-bold transition-colors inline-flex items-center gap-2 shadow-lg">
              {s.heroButton1Label} <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to={s.heroButton2Link} className="bg-white/15 backdrop-blur-sm border border-white/30 hover:bg-white/25 text-white px-8 py-3 rounded text-sm font-body font-semibold transition-colors">
              {s.heroButton2Label}
            </Link>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 text-white/75 text-sm font-body">
            {s.heroBadges.map((badge, i) => (
              <span key={i} className="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-1.5">
                {i === 0 && <CalendarDays className="w-4 h-4 text-yellow-300" />}
                {i === 1 && <MapPin className="w-4 h-4 text-yellow-300" />}
                {i === 2 && <Users className="w-4 h-4 text-yellow-300" />}
                {i > 2 && <CalendarDays className="w-4 h-4 text-yellow-300" />}
                {badge}
              </span>
            ))}
          </div>
      ...
      <section className="py-16 md:py-20">
        <div className="container mx-auto px-6 max-w-3xl text-center">
          <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground mb-6">{s.aboutPreviewHeading}</h2>
      ...
      <section className="bg-section-alt py-16 md:py-20">
        <div className="container mx-auto px-6">
          <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground text-center mb-12">{s.featuresHeading}</h2>
      ...
      <section className="bg-section-alt py-16 md:py-20">
        <div className="container mx-auto px-6">
          <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground text-center mb-4">{s.editionsHeading}</h2>
      ...
      <section className="py-16 md:py-20">
        <div className="container mx-auto px-6 text-center">
          <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground mb-4">{s.ctaHeading}</h2>
          <p className="text-muted-foreground font-body mb-8 max-w-xl mx-auto">
            {s.ctaText}
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to={s.ctaButton1Link} className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-3 rounded text-sm font-body font-semibold transition-colors">
              {s.ctaButton1Label}
            </Link>
            <Link to={s.ctaButton2Link} className="border border-border hover:bg-muted text-foreground px-6 py-3 rounded text-sm font-body font-semibold transition-colors">
              {s.ctaButton2Label}
            </Link>
          </div>
        </div>
